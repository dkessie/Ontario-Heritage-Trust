require([
...
], function(Map, MapView, GraphicsLayer, Graphic, Point, Polygon, SimpleMarkerSymbol, SimpleFillSymbol, SimpleLineSymbol, PopupTemplate, Locate, ScaleBar, Fullscreen, Extent, geometryEngine) {
    ...
    const symbols = {
        ...
        highlighted: new SimpleMarkerSymbol({
            color: [255, 193, 7, 0.9],
            size: 16,
    ...
    // MAKE initializeApp GLOBAL
    window.initializeApp = function() { // <--- MODIFICATION HERE
        showLoadingScreen();
        createMap();
        setupEventListeners();
        loadData()
            .then(() => {
                console.log("Data loaded and processed. Handling URL parameters.");
                handleUrlParameters();
            })
            .catch(err => {
    ...
    function showLoadingScreen() {
        ...
        if (!loadingScreen) { // Guard clause in case map section isn't visible yet
    ...
    function updateTopProgressBar(percentage) {
        ...
        if (!topProgressBar) return; // Guard if it wasn't found/shown
    ...
    function processData() {
        ...
        if (view && view.ready) {
             setTimeout(() => {
                if (filteredFeatures.length > 0) zoomToAllFeatures();
            }, 1000);
        }
    }
    ...
    function createFeature(placemark, isPlaque) {
        ...
        if (!view || !view.spatialReference) {
            console.warn("View or spatialReference not ready for creating feature.");
            return null;
        }
        ...
        if (geometryData.type === 'Point') {
            ...
            symbol = isPlaque ? symbols.plaque : symbols.propertyPoint;
            ...
            esriGeometry = new Polygon({ rings: rings, spatialReference: view.spatialReference });
    ...
    function populateFilters() {
        ...
        if (regionSelect) {
            ...
            Array.from(regions).sort().forEach(region => {
                ...
                option.value = region;
                option.textContent = region; // Potentially translate common region names
        ...
        if (featureName) {
            ...
            if (featureToSelect) {
                if (view) {
                    ...
                    // Wait for view to be ready if called too early
    ...
    // This function will be called by app.js when language changes
    window.updateMapUIText = function() {
        // Re-translate static-like text if needed, for example, default options in selects.
        // Or titles of map controls if they were set programmatically and not via data-i18n-title-key.
        const searchInput = document.getElementById('search-input');
        if (searchInput) searchInput.placeholder = window.getTranslatedString('mapApp.searchPanel.placeholder');

        // Update basemap names
        document.querySelectorAll('.basemap-option').forEach(option => {
        ...
        // Update filter "All" options if they are static in HTML but need re-translation
        const regionFilter = document.getElementById('region-filter');
        ...
        // Refresh feature types (used in createFeature and potentially elsewhere)
        // This is important if a feature popup is open or search results are visible when lang changes
        if (typeof performSearch === 'function' && document.getElementById('search-input')?.value) performSearch(); // Re-run search to update types in results
        ...
        if (currentPopup && currentPopup.classList.contains('show') && currentPopup.currentFeature) {
            // Temporarily update the 'type' attribute for the current feature for popup display
            const feature = currentPopup.currentFeature;
            feature.attributes.type = feature.attributes.originalData.type === 'plaque' // Assuming originalData stores original type
                ? window.getTranslatedString('mapApp.filterPanel.plaques')
                : window.getTranslatedString('mapApp.filterPanel.properties');
            showFeaturePopup(feature); // Re-render popup with new language
        }
    };


    // DO NOT CALL initializeApp() directly here anymore.
    // app.js will call window.initializeApp() when the map section is shown.
